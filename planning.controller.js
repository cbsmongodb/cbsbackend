...
export async function getPlanning(req, res) {
  try {
    const plan = await PlanConfiguration.findById(req.params.id).populate(POPULATE);
    ...
    res.json({ ...plan.toObject(), planConfigurationDoctors: doctors, doctors: doctors.map(d => d.doctor).filter(Boolean) });
  } catch (err) {
